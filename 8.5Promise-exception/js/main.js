...
(function () {
	...
	app.onloaded = function () {
	    document.getElementById("raiseError").addEventListener("click", fireAnError);
	    document.getElementById("raiseException").addEventListener("click", fireAnException);
	}

	app.start();

	function fireAnError() {
	    // register the error handler
	    WinJS.Promise.onerror = errorHandler.bind(this);
	...
	function errorHandler(evt) {
	    ...
	    clearOutput();
	    output(" Properties of the argument of the error handler");

	    // sort through the detail contents to identify which one's are from promises
	    for (var i in detail) {
	        if (i !== "promise") {
	            output("<br/>Property '" + i + "' : " + detail[i]);
	        } else {
	            output("<br/>Property '" + i + "' : promise object");
	        }
	    }
